...
class PlaygroupSearch extends Component {
  ...
  handleFormSubmit = event => {
    ...
    if (
      ...
      (cityInput.trim().length > 0 && stateInput.trim().length > 0) ||
      stateInput.trim().length > 0 ||
      (stateInput.trim().length === 0 &&
        cityInput.trim().length === 0 &&
        nameInput.trim().length === 0)
    ) {
      API.findPlaygroups(nameInput, cityInput, stateInput)
        .then(res => {
  ...
  render() {
    return [
            ...
            <FormGroup style={{ margin: "15px" }}>
              ...
              <InputGroup>
                ...
                >
                  ...
                  <option disabled key={"break"} value={null} />
                  {states.map(state => (
              ...
              <Button bsStyle="dark" type="submit">
                Search
              </Button>
            </FormGroup>
          </Form>
      ...
      <Panel.Body
        ...
        key="results"
        style={{ margin: "0px", background: "#dcdcdc" }}
      >
      ...
      <Row bsClass="row-fluid">
        ...
        <Col sm={6} smOffset={3} md={3} mdOffset={0} lg={2} lgOffset={0}>
          <Button
            ...
            style={{ width: "100%" }}
            onClick={event => {
              API.requestGroupJoin(props.groupId)
                .then(response => {
