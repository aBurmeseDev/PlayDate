...
class PlaygroupChat extends Component {
  ...
  componentDidMount() {
    ...
    this.socket.on("chat message", messageData => {
      try {
        this.setState({
          messages: this.state.messages.concat(messageData.message)
        });
      } catch (err) {}
    });
    ...
    const top =
      this.messagesElement.getBoundingClientRect().top + window.scrollY;
  ...
  sendMessage = () => {
    if (
      ...
      this.state.messageInput.trim().length > 0
    ) {
  ...
  removeRequest = userId => {
    const newRequests = this.state.group.requests.filter(
  ...
  render() {
    return [
      <ListGroup key="main-body">
        <ListGroupItem>
          <Row>
            ...
            <Col xs={2} sm={2} md={1} lg={1}>
              {this.isGroupOwner() && this.props.group.requests.length > 0 && (
                <RequestsButton
                  {...this.props}
        ...
        <div
          className="list-group-item panel-group"
          style={{
    ...
    <OverlayTrigger
      ...
      overlay={
        <Popover className="container list-group" id="requests-popover">
          {props.group.requests.map(request => (
            <Request
              key={request._id}
              group={props.group}
              user={request}
