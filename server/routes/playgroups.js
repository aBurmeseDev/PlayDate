...
router.get("/find", (req, res) => {
  ...
  if (queryData.city)
    params.$where =
      "this.city.toLowerCase() === '" + queryData.city.toLowerCase() + "'";
  if (queryData.name) {
    if (params.$where) params.$where += " && ";
    else params.$where = "";
    params.$where +=
      "this.name.toLowerCase().includes('" +
      queryData.name.toLowerCase() +
      "')";
  }

  db.Playgroup.find(params)
    .populate("owners members requests")
    .then(results => {
...
router.get("/joined", (req, res) => {
  ...
  db.Playgroup.find({ $or: [{ owners: user._id }, { members: user._id }] })
    .populate("owners members requests")
    .then(results => {
      //console.log(results);
      res.status(200).json(results);
...
router.put("/request", (req, res) => {
  ...
  const queryData = url.parse(req.url, true).query;
  if (!user || !queryData) return res.status(401).end();
  db.Playgroup.findOneAndUpdate(
    { _id: queryData.groupId },
...
router.post("/confirm-request", (req, res) => {
  ...
  db.Playgroup.findOneAndUpdate(
    { _id: queryData.groupId },
    {
      $addToSet: { members: queryData.userId },
      $pull: { requests: queryData.userId }
    },
    { new: true }
  )
    .then(result => {
